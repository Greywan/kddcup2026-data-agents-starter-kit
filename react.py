...
try:
    ...
    logger.info("[agent_loop] Using tiktoken for accurate token counting")
except Exception as e:
    logger.info(f"[agent_loop] Warning: Failed to load tiktoken encoder: {e}")
    _tokenizer = None
...
def _extract_tool_call(content: str) -> str | None:
    ...
    if func_match:
        ...
        current_logger.info(f"[agent] Converted <function={tool_name}> to JSON: {result}")
        return result
    ...
    for attempt in range(3):
        try:
            ...
            if full and full.strip():
                return full.strip()
            logger.info(f"[agent] Empty response on attempt {attempt + 1}")
        except Exception as e:
            err_str = str(e)
            logger.info(f"[agent] LLM call attempt {attempt + 1} failed: {e}")
            # Content safety filter - no point retrying the same prompt
            if "data_inspection_failed" in err_str:
                logger.info(f"[agent] Content filter triggered, signaling redirect")
...
async def _execute_tool(tool_name: str, tool_args: dict, logger) -> str:
    ...
    try:
        if tool_name == "search":
            ...
            if isinstance(engines, str):
                engines = [engines]
            result = batch_search(queries, engines=engines, logger=logger)
...
async def react_agent(question: str, workflow_logger=None) -> str:
    """
    ...
    current_logger = workflow_logger if workflow_logger else logger
    ...
    for round_idx in range(MAX_ROUNDS):
        ...
        if elapsed > TIMEOUT_SECONDS:
            current_logger.info(f"[agent] Timeout after {elapsed:.0f}s at round {round_idx + 1}")
            return await _force_answer(messages)

        # Call LLM
        current_tokens = _estimate_tokens(messages)
        current_logger.info(f"[agent] Round {round_idx + 1}, elapsed {elapsed:.0f}s, tokens {current_tokens}")
        content = await _call_llm(messages, logger=current_logger)
        current_logger.info(f"[agent] LLM response: {content}")

        # Handle content filter: redirect or force a neutral search
        if content == CONTENT_FILTER_MARKER:
            content_filter_count += 1
            current_logger.info(f"[agent] Content filter hit #{content_filter_count} at round {round_idx + 1}")
            if content_filter_count <= 2:
                ...
                current_logger.info(f"[agent] Too many content filters, forcing answer with sanitized context")
        ...
        if len(content.strip()) < 5 and "<answer>" not in content:
            current_logger.info(f"[agent] Stalled output detected: '{content.strip()}', retrying")
            continue

        messages.append({"role": "assistant", "content": content})

        # Debug: current_logger.info first 500 chars of model output
        current_logger.info(f"[agent] Output: {content}")
        ...
        if "<answer>" in content:
            if "</answer>" in content:
                ...
                current_logger.info(f"[agent] Detected <answer> without </answer>, extracted: {answer_text}")
            ...
            if answer_text:
                ...
                if round_idx < 3 and elapsed < 120 and not early_answer_blocked:
                    ...
                    current_logger.info(f"[agent] Early answer blocked at round {round_idx + 1}, requesting verification")
                    continue

                current_logger.info(f"[agent] Got answer at round {round_idx + 1}: {answer_text}")
                return _normalize_answer(answer_text)
        ...
        if tool_call_str:
            try:
                ...
                current_logger.info(f"[agent] Calling tool: {tool_name}")

                result = await _execute_tool(tool_name, tool_args, current_logger)
        ...
        if _estimate_tokens(messages) > MAX_TOKENS_ESTIMATE:
            current_logger.info(f"[agent] Token limit reached at round {round_idx + 1}")
            return await _force_answer(messages)

    # Exhausted all rounds
    current_logger.info(f"[agent] Max rounds ({MAX_ROUNDS}) exhausted")
    return await _force_answer(messages)
